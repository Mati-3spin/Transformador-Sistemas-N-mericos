...
- Conversión **Decimal → Hexadecimal** (1 a 255)
...
Todo el programa vive en un único archivo `binario.py`, con un bucle principal `while True` que despacha la opción elegida a un bloque `if/elif` por cada conversión. Las conversiones usan tuplas de valores posicionales (`Prefijo_bin`, `prefijo_bin`) y diccionarios de mapeo (`prefijo_hexa`, `prefijo_Hexa`) para traducir entre sistemas.
...
- La opción 5 no permite convertir el número `0` (el rango validado es estrictamente mayor a 0).
- El historial usa el valor de entrada como clave, así que una misma entrada repetida en distintas opciones se sobrescribe.

## 📝 Licencia

Este proyecto es de uso libre. Puedes agregar aquí la licencia que prefieras (MIT, GPL, etc.).

## 👤 Autor

Proyecto personal — hecho con Python 🐍
